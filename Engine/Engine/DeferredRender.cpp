...
DeferredRender::DeferredRender(Game& game, const std::vector<vk::Image>& swapchain_images)
    : m_game(game)
{
    // TODO: init m_swapchain_data 
    // TODO: init m_swapchain_data.m_color_image
    ...
    InitializeConstantPerImage(swapchain_images);
...
{
    // TODO: init m_materials
    // TODO: init materials_by_type
    // TODO: init mesh_by_material
    InitCommandBuffer();
}
...
void DeferredRender::InitializeDeferredPipelineLayout()
{
    ...
    m_deffered_descriptor_set_layouts = {
        m_game.get_device().createDescriptorSetLayout(vk::DescriptorSetLayoutCreateInfo({}, bindings))
    };
    m_deffered_layout = m_game.get_device().createPipelineLayout(vk::PipelineLayoutCreateInfo({}, m_deffered_descriptor_set_layouts, {}));
}
...
void DeferredRender::InitializeConstantPerImage(const std::vector<vk::Image>& swapchain_images)
{
...
void DeferredRender::InitializeDeferredPipeline()
{
    ...
    auto pipeline_result = m_game.get_device().createGraphicsPipeline(m_deffered_cache, vk::GraphicsPipelineCreateInfo({}, stages, &vertex_input_info, &input_assemply, {}, &viewport_state, &rasterization_info, &multisample, &depth_stensil_info, &blend_state, &dynamic, m_deffered_layout, m_deffered_render_pass));
