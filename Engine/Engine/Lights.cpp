	...
	: m_game(game), m_ProjectionMatrix(ProjectionMatrix)
{
	...
	for (int i = 0; i < m_swapchain_data.size(); ++i) {
		...
		m_swapchain_data[i].m_depth_image_view = m_game.get_device().createImageView(vk::ImageViewCreateInfo({}, m_swapchain_data[i].m_depth_image, vk::ImageViewType::e2D, m_game.get_depth_format(), vk::ComponentMapping(), vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eDepth, 0, 1, 0, m_lights.size() + 1)));
