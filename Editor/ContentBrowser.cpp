...
namespace Editor {
	...
	void ContentBrowser::Render(bool* p_open, ImGuiWindowFlags flags) {
		...
		if (m_CurrentDirectory.empty()) {
			ImGui::Text("Project must be saved.");
		} else {
	...
	void ContentBrowser::SetContext(EDITOR_GAME_TYPE ctx) {
		Editor::GameWidget::SetContext(ctx);

		if (GameProject::Instance()->HasSourceRoot()) {
			m_CurrentDirectory = GameProject::Instance()->GetSourceRoot();
		} else {
			m_CurrentDirectory = std::filesystem::path();
		}
	}
